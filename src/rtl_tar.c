...
#ifndef NO_CREATE
static int create(const char *filename, int compress, const char **argv);
#endif
static int extract(const char *filename, int do_extract, int flags);
static int copy_data(struct archive *, struct archive *);

static int verbose = 0;

int rtl_tar(const char *cmd)
{
	const char *filename = NULL;
	int compress, flags, mode, opt;
	char *p, *cp, *save_cp;
	int n_spaces = 0, ret = -1;;
	const char **argv = NULL, **save_argv = NULL, **tmp;

	/* convert string to arrary */
	if ((cp = strdup(cmd)) == NULL)
		return -1;
	save_cp = cp;

	while ((p = strsep(&cp, " ")) != NULL) {
		tmp = realloc(argv, sizeof(char *) * ++n_spaces);
		if (!tmp) {
			save_argv = argv;
			goto out;
		}
	...
	tmp = realloc(argv, sizeof(char *) * (n_spaces + 1));
	if (!tmp) {
		save_argv = argv;
		goto out;
	}
...
#ifndef NO_CREATE
	case 'c':
		ret = create(filename, compress, argv++);
		break;
#endif
	case 't':
		ret = extract(filename, 0, flags);
		break;
	case 'x':
		ret = extract(filename, 1, flags);
		break;
	}
