...
static void sha256_transform(rtl_sha256_ctx *ctx, const uint8_t data[])
{
...
{
	rtl_sha256_ctx ctx;
	unsigned char sha256[RTL_SHA256_BLOCK_SIZE];

	if (size < RTL_SHA256_BLOCK_SIZE * 2 + 1)
		return -1;

	rtl_sha256_init(&ctx);
	rtl_sha256_update(&ctx, (uint8_t *)str, strlen(str));
	rtl_sha256_final(&ctx, sha256);

	int i;
	for (i = 0; i < RTL_SHA256_BLOCK_SIZE; i++)
...
int rtl_sha256_file(const char *filename, char *result, size_t size)
{
	...
	while ((len = fread(buffer, 1, sizeof(buffer), fp)) > 0)
		rtl_sha256_update(&ctx, buffer, len);
