...
int rtl_readw_lock(int fd)
{
	...
	struct flock lock;
	rtl_lock_init(&lock, F_RDLCK, SEEK_SET, 0, 0);
...
int rtl_writew_lock(int fd)
{
	...
	struct flock lock;
	rtl_lock_init(&lock, F_WRLCK, SEEK_SET, 0, 0);
...
int rtl_unlock(int fd)
{
	...
	struct flock lock;
	rtl_lock_init(&lock, F_UNLCK, SEEK_SET, 0, 0);
...
{
	struct flock lock;
	rtl_lock_init(&lock, type, whence, start, len);
